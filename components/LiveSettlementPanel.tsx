    ...
    <div className="border border-border p-4">
      ...
      <div className="flex flex-col gap-2 mb-4">
        {participants.map(participant => (
          <div key={participant.player_id} className="grid grid-cols-[minmax(0,1fr)_5rem] items-center gap-x-3 gap-y-1 sm:flex sm:gap-3">
            <span className="min-w-0 flex-1 truncate text-sm text-white">{participant.name}</span>
            <span className="col-start-1 row-start-2 text-xs text-muted sm:col-auto sm:row-auto">buy-in {participant.total_buyin.toLocaleString()}</span>
            {isCashedOut(participant) ? (
              <div className="col-start-2 row-span-2 row-start-1 w-20 border border-transparent px-2 py-2 text-right sm:col-auto sm:row-auto sm:w-28 sm:px-3">
              ...
              <input type="number" inputMode="numeric" min="0" value={finals[participant.player_id] ?? ''}
                ...
                placeholder="final"
                className="col-start-2 row-span-2 row-start-1 w-20 border border-border bg-surface px-2 py-2 text-right text-sm text-white outline-none transition-colors placeholder:text-muted focus:border-white sm:col-auto sm:row-auto sm:w-28 sm:px-3" />
            )}
          </div>
        ))}
      </div>

      <div className="mb-4 flex flex-col gap-2 border-t border-border pt-3 text-xs tracking-widest sm:flex-row sm:items-center sm:justify-between">
