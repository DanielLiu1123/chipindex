          ...
          <tbody>
            ...
            {sortedStats.map((s, i) => (
              <tr key={s.player.id} className="border-b border-border hover:bg-surface transition-colors">
                ...
                <td className="py-4">
                  <Link href={`/groups/${groupId}/players/${s.player.id}`} className="block">{s.player.name}{s.group_player.deleted_at && <span className="ml-2 text-[10px] text-muted">INACTIVE</span>}</Link>
                </td>
