...
export async function createGroup(name: string): Promise<Group> {
  ...
  if (!trimmed) throw new DomainError('invalid_input', 'Name required')
  const { data, error } = await db.from('group').insert({ name: trimmed }).select().single()
  if (error?.code === '23505') throw new DomainError('conflict', 'Group name already exists')
  ensure(error)
  if (!data) throw new Error('Database write returned no data')
  return data
}
...
async function insertGroupPlayer(groupId: string, playerId: string): Promise<GroupPlayer> {
  ...
  if (error?.code === '23505') throw new DomainError('conflict', 'Player already exists in group')
  ensure(error)
  if (!data) throw new Error('Database write returned no data')
  return data
}
...
export async function deleteGroupPlayer(groupId: string, playerId: string): Promise<GroupPlayer> {
  ...
  if (!data) throw new DomainError('not_found', 'Group player not found')
  return data
}
...
export async function createPlayer(name: string, groupId: string): Promise<{ player: Player; group_player: GroupPlayer }> {
  ...
  if (!player) throw new Error('Database write returned no data')
  try {
    return { player, group_player: await insertGroupPlayer(groupId, player.id) }
  } catch (cause) {
    const timestamp = now()
    const { error: cleanupError } = await db.from('player').update({ deleted_at: timestamp, updated_at: timestamp }).eq('id', player.id)
    if (cleanupError) throw new AggregateError([cause, cleanupError], 'Player creation and cleanup failed')
    throw cause
  }
}
...
export async function renamePlayer(groupId: string, id: string, name: string): Promise<Player> {
  ...
  if (!membership) throw new DomainError('not_found', 'Player not found in group')
