...
export async function cashOutParticipant(groupId: string, sessionId: string, playerId: string, finalChips: number) {
  ...
  if (participant.settled_at !== null) throw new DomainError('conflict', 'Participant has already cashed out')
  const timestamp = now()
  ...
  if (!data) throw new DomainError('conflict', 'Participant has already cashed out')
  return data
}
...
export async function undoParticipantCashOut(groupId: string, sessionId: string, playerId: string) {
  ...
  await requireOpenSession(groupId, sessionId)
  const participant = await requireParticipantSettlementState(sessionId, playerId)
  if (participant.settled_at === null) throw new DomainError('conflict', 'Participant has not cashed out')
  const timestamp = now()
  const { data, error } = await db.from('session_participant')
...
export async function revokeBuyin(groupId: string, sessionId: string, buyinId: string): Promise<void> {
  ...
  if (!buyIn) throw new DomainError('not_found', 'Buy-in not found')
  const participant = await requireParticipantSettlementState(sessionId, buyIn.player_id)
  if (participant.settled_at !== null) throw new DomainError('conflict', 'Cashed-out participant cannot revoke buy-ins')
  const timestamp = now()
...
export async function settleSession(groupId: string, sessionId: string, finals: FinalEntry[], force: boolean): Promise<{ id: string; diff: number }> {
  ...
  if (!participants.length) throw new DomainError('invalid_input', 'No participants')

  requireUniquePlayerIds((finals ?? []).map(final => final.player_id))
  const requestedFinals = new Map<string, number>()
  for (const final of finals ?? []) {
    if (!Number.isInteger(final.final_chips) || final.final_chips < 0) throw new DomainError('invalid_input', `Invalid final_chips for ${final.player_id}`)
  ...
  for (const participant of participants) {
    if (participant.settled_at !== null) {
      if (!Number.isInteger(participant.final_chips) || participant.final_chips == null || participant.final_chips < 0) {
        throw new Error(`Cashed-out participant ${participant.player_id} has invalid final_chips`)
      }
      const requested = requestedFinals.get(participant.player_id)
      if (requested !== undefined && requested !== participant.final_chips) throw new DomainError('conflict', `Frozen final_chips do not match for participant ${participant.player_id}`)
      finalByPlayer.set(participant.player_id, participant.final_chips)
      continue
    }
    const requested = requestedFinals.get(participant.player_id)
    if (requested === undefined) throw new DomainError('invalid_input', `Missing final_chips for participant ${participant.player_id}`)
  ...
  for (const result of participantResults) {
    ...
    if (!result.data) throw new DomainError('conflict', 'Participant state changed during settlement')
  }
